...
Metrics produced
────────────────
  Per class (matching Table 2 of the paper):
    • Top-1 accuracy   (= recall per class, TP / (TP + FN))
    • Precision        (TP / (TP + FP))
  ...
  Aggregate:
    • Weighted-average accuracy, precision, FPR, F1
    • Overall accuracy

  Outputs saved to <output_dir>/assessment/:
    • assessment_report.txt   — full text table (paper Table-2 style)
    • confusion_matrix.png    — normalised heatmap
    • per_class_metrics.png   — grouped bar chart
    • metrics.json            — machine-readable dict

  Detection metrics (from YOLO validation, if provided):
    • mAP@0.5, mAP@0.5-0.95, precision, recall
    • Saved to assessment_report.txt
"""
...
class ClassifierAssessment:
    """
    Evaluates the trained ResNeXt-50 classifier on a crop-patch dataset,
    producing the same metrics table as DeepWeeds Table 2.
    """
    ...
    def evaluate(
        ...
        """
        Run the classifier on `dataset`, collect predictions and labels,
    ...
    def _compute_metrics(self, y_true: np.ndarray, y_pred: np.ndarray) -> Dict:
        ...
        for i, name in enumerate(self.class_names):
            tp = cm[i, i]
            fn = cm[i, :].sum() - tp          # missed true positives
            fp = cm[:, i].sum() - tp           # false positives for class i
            tn = cm.sum() - tp - fn - fp

            support  = int(cm[i, :].sum())
            recall   = tp / (tp + fn) if (tp + fn) > 0 else 0.0   # = top-1 acc
    ...
    def save_report(self, metrics: Dict,
                    yolo_metrics: Optional[Dict] = None,
                    split_name: str = "test"):
    ...
    def _save_text_report(self, metrics: Dict,
        ...
        if yolo_metrics:
            lines += [
                "─" * 80,
                "  YOLO DETECTION METRICS (validation set)",
                "─" * 80,
    ...
    def _save_confusion_matrix(self, metrics: Dict):
    ...
    def _save_per_class_chart(self, metrics: Dict):
        pc     = metrics["per_class"]
        ...
        # Reference line from DeepWeeds paper
    ...
    def _save_json(self, metrics: Dict, yolo_metrics: Optional[Dict]):
        data = dict(metrics)
        if yolo_metrics:
...
# ── DeepWeeds dataset note ────────────────────────────────────────────────────

DEEPWEEDS_DOWNLOAD_NOTE = """
DeepWeeds dataset (Olsen et al., 2019) — download instructions
──────────────────────────────────────────────────────────────
  GitHub : https://github.com/AlexOlsen/DeepWeeds
  Zenodo : https://doi.org/10.5281/zenodo.3370734

  Quickstart:
    pip install zenodo-get
    zenodo_get 3370734            # downloads images.zip + labels.csv
    unzip images.zip -d deepweeds/images
    # labels.csv has columns: Filename, Label, Species

  Then point the pipeline at it:
    python run_pipeline.py --dataset_dir /path/to/deepweeds

  The dataset has 9 classes (8 weed species + Negative):
  ...
  Note: DeepWeeds is image-level classification (no bounding boxes).
  The YOLO detection stage will be skipped or run with a placeholder.
  Set --skip_yolo in run_pipeline.py to go straight to classification.
"""
